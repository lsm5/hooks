...
{
	_cleanup_(json_decrefp) json_t *node = NULL;
	...
	const char *container_uuid = json_string_value(v_id);
	...
	snprintf(config_file_name, sizeof(config_file_name), "%s/config.json", json_string_value(v_bundle_path));
	...
	if (config_node == NULL) {
		if (strlen(err.text)) {
			pr_perror("%s: parse_error: %s", id, err.text);
		} else {
	...
	json_t *v_args = json_object_get(config_node, *args_path);
	if (!v_args) {
	...
	json_t *v_envs = json_object_get(config_node, *envs);
	if (v_envs) {
                json_array_foreach(v_envs, i, n) {
			json_t *v_env = json_array_get(v_envs, i);
			const char *str = json_string_value(v_env);
			/****
...
#if ARGS_CHECK
	const char *cmd = NULL;
	json_t *v_arg0_value = json_array_get(v_args, sizeof(v_args));
	...
	const char *rootfs = json_string_value(v_root);
	...
	if (! config_mounts) {
        ...
        json_array_foreach(v_mounts, i, n) {
		json_t *v_mounts_values = json_array_get(v_mounts, i);
		...
		config_mounts[i] = json_string_value(v_destination);
	}
	...
	*/
	...
	if ((stage != NULL && !strcmp(stage, "prestart")) ||
		...
		if (gid != 0) {
			array_len = json_array_size(v_gidMappings);
			if (array_len < 1) {
				pr_perror("%s: No gid for container found", id);
				return EXIT_FAILURE;
			}

                        json_array_foreach(v_gidMappings, i, n) {
				json_t *v_gidMappings_values = json_array_get(v_gidMappings, i);
				json_t *v_containerId = json_object_get(v_gidMappings_values, "containerID");
				if (json_integer_value(v_containerId) == 0) {
					json_t *v_gid = json_object_get(v_gidMappings_values, "hostID");
					gid = v_gid ? json_integer_value(v_gid) : -1;
					i = array_len;
				}
			}
		} /* End if (gid!=0) */
		...
		json_t *v_uidMappings = json_object_get(config_node, *uid_mappings);
		if (!v_uidMappings) {
		...
		if (uid !=0) {
			array_len = json_array_size(v_uidMappings);
			if (array_len < 1) {
				pr_perror("%s: No uid for container found", id);
				return EXIT_FAILURE;
			}

                        json_array_foreach(v_uidMappings, i, n) {
				json_t *v_uidMappings_values = json_array_get(v_uidMappings, i);
				...
				if (json_integer_value(v_containerId) == 0) {
					...
					uid = v_uid ? json_integer_value(v_uid) : -1;
					i = array_len;
				}
			}
		} /* End if (uid !=0) */
